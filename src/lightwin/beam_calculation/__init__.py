...
|BC| is an Abstract Base Class to hold such a solver. :class:`.Envelope1D`,
:class:`.Envelope3D` and :class:`.TraceWin` are the implemented solvers
inheriting from it. They can be created very easily using the
:mod:`.beam_calculation.factory` module.

|SO| is used to uniformly store simulation outputs with all solvers.
